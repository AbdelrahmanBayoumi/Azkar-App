...
public class AudioPlayer {
    ...
    private void playbackLoop() {
        ...
        if (streamSnapshot != null && lineSnapshot != null && state == State.PLAYING) {
            try {
                ...
                if (playbackFailed) Logger.error("Error during audio playback: " + audioFile.getName(), e, "AudioPlayer");
            }
        }
    ...
    public void dispose() {
        ...
        if (stoppedBeforeWorkerStarted) triggerCallbacks(false);
    }

    private static void stopAndFlush(SourceDataLine lineToStop) {
        if (lineToStop == null) return;
        try {
            if (lineToStop.isRunning()) lineToStop.stop();
            lineToStop.flush();
        } catch (Exception | LinkageError | ServiceConfigurationError e) {
    ...
    private void cleanupLineAndStreams() {
        ...
        if (decodedToClose != rawToClose) closeQuietly(rawToClose);
    }
    ...
    private void applyVolumeToLine(float volume) {
        SourceDataLine lineSnapshot;
        synchronized (lock) { lineSnapshot = line; }
        if (lineSnapshot == null || !lineSnapshot.isOpen()) return;
        try {
    ...
    public void setOnEndOfMedia(Runnable callback) { synchronized (lock) { onEndOfMedia = callback; } }
    public void setOnStopped(Runnable callback) { synchronized (lock) { onStopped = callback; } }
    State getState() { return state; }

    public static class AudioPlayerException extends Exception {
        public AudioPlayerException(String message, Throwable cause) { super(message, cause); }
    }
}
