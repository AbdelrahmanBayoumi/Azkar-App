...
public class AudioPlayerTest {
    ...
    private File createValidWavFile(String fileName, int frames) throws Exception {
        ...
        AudioInputStream stream = new AudioInputStream(
                new ByteArrayInputStream(new byte[frames * format.getFrameSize()]), format, frames);
    ...
    private AudioPlayer player(File file, FakeLine line) throws Exception {
        return new AudioPlayer(file, new AudioPlayer.LineProvider() {
            @Override
            public SourceDataLine open(AudioFormat format) {
                line.open(format);
                return line;
            }
    ...
    public void mp3FileOpensThroughSpiAsSignedPcm() throws Exception {
        File mp3 = new File("jarFiles/audio/juntos.mp3");
        assumeTrue("bundled notification mp3 is missing", mp3.isFile());
        FakeLine line = new FakeLine();
        AudioPlayer player = player(mp3, line);
        assertNotNull(line.format);
    ...
    public void missingFileThrowsException() throws Exception {
        new AudioPlayer(new File(tempFolder.getRoot(), "missing.wav"));
    }

    @Test(expected = AudioPlayer.AudioPlayerException.class)
    public void oversizedFileThrowsException() throws Exception {
        File largeFile = tempFolder.newFile("large.wav");
        try (RandomAccessFile file = new RandomAccessFile(largeFile, "rw")) {
    ...
    public void stopWhileDrainBlockedWinsOverNaturalEof() throws Exception {
        ...
        player.play();
        await(line.drainEntered);
        player.stop();
        line.releaseDrain.countDown();
        await(callback);
        assertEquals(1, stopped.get());
    ...
    private static final class FakeLine implements SourceDataLine {
        ...
        @Override
        public void open(AudioFormat value, int bufferSize) {
            open(value);
        }
        ...
        @Override
        public void start() {
            gainSetBeforeStart = gain.wasSet;
            startGain = gain.getValue();
            running = true;
            started.countDown();
        }
        ...
        public boolean isControlSupported(Control.Type type) {
            return FloatControl.Type.MASTER_GAIN.equals(type);
        }
    ...
    private static final class Gain extends FloatControl {
        ...
        Gain() {
            super(FloatControl.Type.MASTER_GAIN, -80.0f, 6.0f, 0.1f, 1, 0.0f, "dB");
        }
